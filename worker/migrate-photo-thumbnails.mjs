// 시설물현황/비품수자원/비품도공/하자보수 4개 앱의 기존 사진들에 대해 목록용 썸네일(80px 표시,
// 원본 160px 리사이즈+~15KB 압축)을 소급 생성해 각 DB의 "*_썸네일" files 속성에 채워 넣는다
// (2026-09-11, 사용자 요청 — 목록 테이블 초기 로딩 속도 개선을 위해 신규 업로드분은 이미
// facility-status.html 등 4개 앱 코드에서 원본+썸네일 동시 업로드로 전환됨. 이 스크립트는 그
// 전환 이전에 이미 올라간 사진들을 일괄 백필한다).
//
// 사용법:
