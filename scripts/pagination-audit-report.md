...
|---|---|---|---|---|---|
| announcement.html | PAGE_SIZE_OPTIONS=[10,15,30,50,100] | announcement_pageSize | buildPageSizeDd, togglePageSizeDd, renderPaginationRow, goToPage | 신형(.page-btn) | 있음 |
| asset-register-dogong.html | PAGE_SIZE_OPTIONS=[10,15,30,50,100] | assetRegisterDogong_pageSize | buildPageSizeDd, togglePageSizeDd, renderPaginationRow, goToPage | 신형(.page-btn) | 있음 |
| asset-register.html | PAGE_SIZE_OPTIONS=[10,15,30,50,100] | assetRegister_pageSize | buildPageSizeDd, togglePageSizeDd, renderPaginationRow, goToPage | 신형(.page-btn) | 있음 |
| corporate-card.html | PS_OPTIONS=[10,15,31,50,100] | corpCard_pageSize | renderDDPS, pickPS | ⚠️ 구형(.pg/.pager) | ⚠️ 없음 |
...
| overtime-work.html | PAGE_SIZE_OPTIONS=[10,20,31,50,100] | overtimeWork_pageSize | buildPageSizeDd, togglePageSizeDd, renderPaginationRow, goToPage | 신형(.page-btn) | 있음 |
| solar-power.html | PAGE_SIZE_OPTIONS=[10,20,31,50,100] | solarPower_pageSize | buildPageSizeDd, togglePageSizeDd, renderPaginationRow, goToPage | 신형(.page-btn) | 있음 |
| vehicle-log.html | PAGE_SIZE_OPTIONS=[10,20,31,50,100] | vehicleLog_pageSize | buildPageSizeDd, togglePageSizeDd, renderPaginationRow, goToPage | 신형(.page-btn) | 있음 |
...
- **corporate-card.html**: localStorage fallback(corpCard_pageSize) 31
- **defect-management.html**: getPageState size=15
- **facility-status.html**: IIFE return 15
- **monthly-inspection-team.html**: PAGE_DEFAULT_SIZE={photo:15, repair:15, photoMulti:15}
- **monthly-inspection.html**: PAGE_DEFAULT_SIZE={photo:15, photoMulti:15}
...
- `tbody-elecinsp`: ❌ 미연결 — 검토 필요
- `tbody-repair`: ✅ 연결됨
- `tbody-photo`: ✅ 연결됨

### monthly-inspection.html
- `tbody-monthly`: ❌ 미연결 — 검토 필요
- `tbody-elec`: ❌ 미연결 — 검토 필요
- `tbody-transformer`: ❌ 미연결 — 검토 필요
- `tbody-generator`: ❌ 미연결 — 검토 필요
- `tbody-photo`: ✅ 연결됨
- `tbody-power`: ❌ 미연결 — 검토 필요
- `tbody-task`: ❌ 미연결 — 검토 필요
...
- CSS 클래스가 다른 앱과 다름(구형 .pg/.pager): corporate-card.html
- "전체" 옵션이 없는 앱: corporate-card.html, monthly-inspection-team.html, monthly-inspection.html
- 서로 다른 옵션배열 조합 수: 4종 (완전 통일이면 1이어야 함)
- 미연결(검토 필요) tbody 총 7건
